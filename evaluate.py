...
def walk_forward_backtest(
    ...
    `refit_every_days` controls how often the model is retrained: with the
    default of 7 it refits weekly, reusing the last fit for matches in between
    (each match is still predicted from a model trained only on earlier games,
    so there is no look-ahead). Set it to 1 to refit on every match date.
    """
    ...
    for d in unique_dates:
        ...
        need_refit = (
            ...
            or (d - last_fit_date).days >= refit_every_days
        )
        if need_refit:
            candidate = DixonColesModel(max_goals=max_goals)
            weights = (
